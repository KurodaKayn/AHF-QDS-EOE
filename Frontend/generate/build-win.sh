#!/bin/bash
# 在Docker中构建Windows版本的Electron应用
# 作者: KurodaKayn
# 日期: $(date +"%Y-%m-%d")
...
echo -e "${YELLOW}准备在Docker中构建Windows版本...${NC}"
...
if ! docker images | grep -q "electronuserland/builder" | grep -q "wine"; then
...
docker run --rm -ti \
  ...
  ${DOCKER_IMAGE} \
  /bin/bash -c "cd /project && npm install && npm run build:electron:win"
