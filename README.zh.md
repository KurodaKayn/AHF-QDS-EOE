...
![Axum](https://img.shields.io/badge/Axum-0.8-00ADD8?logo=rust&logoColor=white)
...
本项目的设计目的是提高学生效率，在相同时间内能提升到一个更高的做题水平。适合直接把自己从各种地方 Ctrl C + Ctrl V 或者是说爬取的题目拿来反复练习最终达到一个比较理想的死记硬背水平，从而在考试或者是说八股面试中拿到个更好的成绩。

## 支持的操作系统

- **macOS**: 支持 macOS 10.12+
- **Windows**: 支持 Windows 10 和 11
- **Linux**: 暂不支持
...
在 repo 的 Releases 中根据操作系统下载对应的安装包。
...
如果 macOS 遇到如图所示的安全提示问题，可以输入以下命令来解决：

```bash
xattr -rd com.apple.quarantine /Applications/AHF\ QDS\ EOE.app
```

![macOS 安全提示](image.png)

### 3. (可选) 获取 AI API 密钥

要使用 AI 导入功能，请从 DeepSeek 或阿里云百炼获取 API 密钥，并在应用设置中配置。

**推荐使用**: 阿里云百炼 - 有免费额度而且回复更快。

- 链接: `https://bailian.console.aliyun.com/`
- 注意: 建议薅完羊毛就撤退，别真付费。后续会增加更多可以白嫖的 API。

## 技术栈

使用 Next.js 作为全栈框架，最终用 Tauri 打包生成桌面应用。Tauri 使用 Rust 作为后端，提供更小的应用体积和更好的性能。前端使用静态导出模式，确保与 Tauri 的完美兼容。

- **UI 框架**: Tailwind CSS
- **AI 集成**: 目前支持 DeepSeek 和 Qwen（阿里云）。未来可能会支持更多 AI 提供商。

## 数据存储

因为是网页的技术栈，所以数据是存在浏览器的 localStorage 里的。但是因为最后用 Tauri 打包了，所以数据实际会存在：
...
很多题库的刷错题之类的功能不好用，又不能直接导出。所以我就想做个软件来解决这个问题。大语言模型显然是个不错的解决方案，还可以顺带给判断之类的告诉我这个到底为什么对为什么错，而不是因为你的老师太蠢了，不告诉原因，导致你只能无脑硬背。
...
目前已经有了响应式布局，只要导出就行了，但是 app 的存储实现比较麻烦。

---

# 开发者食用指南

本项目使用 Tauri 打包成桌面应用，可以在 Windows、macOS 和 Linux 系统上运行。如果你不是开发者，直接从 Releases 下载安装包即可。
