...
export const useQuizStore = create<QuizState>()(
  persist(
    (set, get) => ({
      ...
      /**
       * 更新题库信息
       * 云模式下调用API，本地模式直接更新状态并自动保存到localStorage
       */
      updateQuestionBank: async (id, name, description) => {
        if (get().isCloudMode) {
          ...
          // 更新状态后，persist中间件会自动将新状态保存到localStorage
        }
      },
      ...
      deleteQuestionBank: async (id) => {
        if (get().isCloudMode) {
          ...
          // 更新状态后，persist中间件会自动将新状态保存到localStorage，实现题库的删除
        }
      },
      ...
      addQuestionToBank: async (bankId, questionData) => {
        if (get().isCloudMode) {
          ...
          // 更新状态后，persist中间件会自动将新状态保存到localStorage，实现题目的持久化存储
      ...
      generateSimilarQuestions: async (originalQuestions) => {
        ...
        try {
          ...
          console.log('处理后的生成题目:', processedQuestions);
