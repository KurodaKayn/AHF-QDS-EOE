...
export async function exportQuestionBank(options: ExportOptions): Promise<void> {
  ...
  if (isTauriRuntime()) {
    ...
    const response = await invoke<ExportBankResponse>("export_question_bank_to_bytes", {
      request: {
        bank,
        format,
      },
...
export const importExportApi = {
  exportBankToBytes: (request: ExportBankRequest): Promise<ExportBankResponse> =>
    invoke<ExportBankResponse>("export_question_bank_to_bytes", { request }),

  importBankFromBytes: (request: ImportBankRequest): Promise<QuestionBank> =>
    invoke<QuestionBank>("import_question_bank_from_bytes", { request }),
