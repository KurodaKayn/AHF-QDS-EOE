...
export default function ReviewPage() {
  ...
  const processQuestionForExplanation = async (questionId: string) => {
    ...
    try {
      ...
      if (questionInfo.options && questionInfo.options.length > 0) {
        problemInfo += '- **选项**:\n';
        questionInfo.options.forEach((opt, idx) => { problemInfo += `  - ${String.fromCharCode(65 + idx)}. ${opt.content}\n`; });
      }
      const correctAnswerText = Array.isArray(questionInfo.answer) ? questionInfo.answer.map(ansId => questionInfo.options?.find(opt => opt.id === ansId)?.content || ansId).join(', ') : (questionInfo.type === QuestionType.TrueFalse ? (questionInfo.answer === 'true' ? '正确' : '错误') : questionInfo.answer);
      ...
      setAiError(`题目 "${questionInfo.content.substring(0, 20)}..." 解析生成失败。`);
      setCurrentExplanations(prev => ({ ...prev, [questionId]: 'AI解析生成失败，请稍后再试。' }));
    } finally {
  ...
  const generateExplanationsForSelected = async () => {
    ...
    const { aiProvider, deepseekApiKey, alibabaApiKey } = settings;
    if ((aiProvider === 'deepseek' && !deepseekApiKey) || (aiProvider === 'alibaba' && !alibabaApiKey)) {
      setAiError('请先在应用设置中配置相应的 API 密钥');
      return;
    }
    setAiError(null);
    selectedQuestions.forEach(questionId => processQuestionForExplanation(questionId));
  };
  ...
  const handleGenerateSimilarQuestions = async () => {
    ...
    const selectedItems: WrongQuestionDisplay[] = wrongQuestions.filter(q => selectedQuestions.has(q.id));
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 lg:p-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <header className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800 dark:text-white">错题回顾</h1>
        <p className="text-gray-600 dark:text-gray-400">温故而知新，巩固你的薄弱环节。</p>
      </header>
      ...
      {aiError && (
        <div className="mb-4 p-3 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 rounded-md flex justify-between items-center">
          <span><FaExclamationTriangle className="inline mr-2" />{aiError}</span>
          <button onClick={() => setAiError(null)} className="text-red-500 hover:text-red-700 dark:text-red-300 dark:hover:text-red-100"><FaTimes /></button>
        </div>
      )}
      ...
      <div className="mb-6 p-4 bg-white dark:bg-gray-800 rounded-lg shadow flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            <FaCheck /> {selectedQuestions.size === filteredQuestions.length && filteredQuestions.length > 0 ? '取消全选' : '全选当前'} ({selectedQuestions.size})
          </button>
          <button
            onClick={generateExplanationsForSelected}
            disabled={selectedQuestions.size === 0 || generatingExplanations.size > 0}
          ...
          >
            <FaRobot /> AI生成解析 {generatingExplanations.size > 0 ? `(${generatingExplanations.size}进行中)` : ''}
          </button>
          <button
            onClick={handleGenerateSimilarQuestions}
            disabled={selectedQuestions.size === 0 || generatingSimilarQuestions}
          ...
          >
            <FaSyncAlt /> AI生成相似题 {generatingSimilarQuestions ? '(生成中...)' : ''}
          </button>
        </div>
      ...
      {filteredQuestions.length === 0 && (
        <div className="text-center py-10">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-400">没有找到符合条件的错题。</p>
          {wrongQuestions.length > 0 && <p className="text-sm text-gray-500 dark:text-gray-500">尝试调整搜索词或筛选条件。</p>}
          {wrongQuestions.length === 0 && <p className="text-sm text-gray-500 dark:text-gray-500">太棒了，当前没有错题！</p>}
        </div>
      )}

      <div className={`grid gap-4 ${viewMode === 'options' ? 'md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
      ...
      <SimilarQuestionsModal 
        ...
        onImport={async (questionsToImport, bankId) => {
            ...
            if (result.success) {
                alert(`成功导入 ${result.importedCount} 道题目。${result.skippedCount > 0 ? `跳过 ${result.skippedCount} 道重复题目。` : ''}`);
            } else {
