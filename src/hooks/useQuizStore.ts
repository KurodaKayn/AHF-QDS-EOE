 ...
 * 题库管理系统全局状态管理模块
 * 
 * 该模块是整个应用的核心状态管理层，使用Zustand库实现，提供以下主要功能：
 * 
 * 1. 题库数据管理
 *    - 题库的创建、读取、更新、删除(CRUD)操作
 *    - 题目的添加、编辑、删除
 *    - 题目数据的结构化存储和检索
 *    - 题目重复检测
 * 
 ...
 * 
 * 4. AI增强功能
 *    - AI提供商配置(DeepSeek/阿里巴巴通义千问)
 *    - 相似题目生成
 *    - 生成题目的导入和管理
 * 
 * 5. 数据持久化
 *    - 使用localStorage自动保存用户数据和设置
 *    - 支持本地模式和云模式切换
 *    - 数据云同步机制
 * 
 * 技术实现：
 * - 基于Zustand状态管理库，使用persist中间件实现持久化
 * - 所有操作方法均返回Promise，支持异步操作
 * - 智能状态合并机制，确保新增配置项兼容性
 * - 部分状态选择性持久化，临时UI状态不持久化
 * 
 * 组件关系：
 * 该store被多个组件使用，包括题库管理页面、题目编辑模态框、
 * 练习页面、复习页面以及设置页面等，是整个应用的数据核心。
 */
